...
import { 
  ...
  List, ListOrdered, Undo, Redo, Heading1, Heading2, Heading3, Quote, Type
} from 'lucide-react';
...
const Editor: React.FC = () => {
  ...
  useEffect(() => {
    if (activeChapter?.content && activeChapter.content.replace(/<[^>]*>/g, '').length > 20) {
  ...
  const handleUpdateContent = (e: any) => {
    ...
    // Hide overlay if user starts typing
    if (showEmptyState && newContent.replace(/<[^>]*>/g, '').length > 5) {
        setShowEmptyState(false);
    }
  };
  ...
  const parseHtmlToDocx = (html: string) => {
    ...
    nodes.forEach((node) => {
        if (node.nodeType === Node.ELEMENT_NODE) {
            ...
            let alignment: AlignmentType = AlignmentType.JUSTIFIED; // Default for ABNT
                ...
                <div className="flex items-center justify-between p-6 border-b border-white/10">
                  <div className="flex items-center gap-3">
                    <div className={`p-2 rounded-lg ${
                      plagiarismResult.score < 10 ? 'bg-emerald-500/10 text-emerald-500' :
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    <div className="bg-[#0B0F19] p-4 rounded-lg border border-white/5 text-center">
                      <div className={`text-3xl font-bold mb-1 ${
                         plagiarismResult.score < 10 ? 'text-emerald-500' :
